...
class RegistroOperacionalModel(BaseModel):
    ...
    def listar_vendas(pagina=1, por_pagina=50, categoria_venda=None, data_inicio=None, data_fim=None):
        """
        Lista vendas em trânsito e entregues com paginação usando offset.
        ...
        Returns:
            dict: Dicionário com registros, total e informações de paginação
        """
        ...
        ).order_by(desc(RegistroOperacionalModel.id))
        
        if categoria_venda == 'transito':
            query = query.filter(CargaModel.ticket_emitido == False)
        elif categoria_venda == 'entregue':
            query = query.filter(CargaModel.ticket_emitido == True)
            if data_inicio:
                query = query.filter(RegistroOperacionalModel.data_entrega_ticket >= data_inicio)
            if data_fim:
    ...
    def listar_vendas_filtrar(
        cliente_venda=None,
        nf_venda=None,
        produto_venda=None,
        bitola_venda=None,
        transportadora_venda=None,
        motorista_venda=None,
        placa_venda=None,
        origem_venda=None,
        data_inicio=None,
        data_fim=None,
        ...
        """
        Filtra vendas em trânsito e entregues com paginação usando offset.
        ...
        if categoria_venda == 'transito':
            query = query.filter(CargaModel.ticket_emitido == False)
            if data_inicio:
                query = query.filter(RegistroOperacionalModel.data_cadastro >= data_inicio)
            if data_fim:
                query = query.filter(RegistroOperacionalModel.data_cadastro <= data_fim)
        elif categoria_venda == 'entregue':
            query = query.filter(CargaModel.ticket_emitido == True)
        ...
        if nf_venda:
            query = query.filter(
                or_(
                    RegistroOperacionalModel.numero_nota_fiscal.ilike(f"%{nf_venda}%"),
                    RegistroOperacionalModel.numero_nota_fiscal_excessao.ilike(f"%{nf_venda}%"),
                    RegistroOperacionalModel.numero_nota_fiscal_estorno.ilike(f"%{nf_venda}%"),
                )
            )
        
        if produto_venda:
            query = query.join(ProdutoModel, CargaModel.produto_id == ProdutoModel.id).filter(
                ProdutoModel.id == produto_venda
            )
        
        if bitola_venda:
            query = query.join(BitolaModel, CargaModel.bitola_id == BitolaModel.id).filter(
                BitolaModel.id == bitola_venda
            )
        
        if transportadora_venda:
            query = query.join(TransportadoraModel, CargaModel.transportadora_id == TransportadoraModel.id).filter(
                TransportadoraModel.identificacao.ilike(f"%{transportadora_venda}%")
            )
        
        if motorista_venda:
            query = query.join(MotoristaModel, CargaModel.motorista_id == MotoristaModel.id).filter(
                MotoristaModel.nome_completo.ilike(f"%{motorista_venda}%")
            )
        
        if placa_venda:
            query = query.join(VeiculoModel, CargaModel.veiculo_id == VeiculoModel.id).filter(
