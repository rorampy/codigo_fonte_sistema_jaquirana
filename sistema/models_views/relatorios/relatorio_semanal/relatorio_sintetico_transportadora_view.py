...
@app.route(
    "/relatorios/relatorio-semanal/sintetico-transportadora", methods=["GET", "POST"]
)
@login_required
@requires_roles
def relatorio_sintetico_transportadora():
    ...
    if request.method == "POST":
        if any(request.form.values()) and not (request.form.get("exportar_pdf") or request.form.get("exportar_excel")):
            ...
            if any([tipo_filtro, semana_selecionada, data_inicio_form, data_fim_form, clienteFiltro, produtoFiltro, bitolaFiltro]):
                ...
                tipo_filtro = "semanal"  # Define tipo padrão para exportações
        ...
        if any(request.args.values()):
            ...
            tipo_filtro = "semanal"  # Define tipo padrão
            if semana_atual_info:
                data_inicio = semana_atual_info["inicio"]
                data_fim = semana_atual_info["fim"]
            else:
    ...
    if request.method == "POST" and request.form.get("exportar_pdf"):
        ...
        html = render_template(
            "/relatorios/relatorio_semanal/relatorio_sintetico_transportadora/exportar_relatorio_sintetico_transportadora_pdf.html",
            logo_path=logo_path,
            dataHoje=dataHoje,
            registros=registros,
        ...
        nome_arquivo_saida = f"relatorio-sintetico-transportadora_{dataHoje}"
    ...
    if request.method == "POST" and request.form.get("exportar_excel"):
        ...
        for item in registros:
            transportadora = item.get("transportadora") or "Não informada"
            peso = item["registro"].peso_liquido_ticket or 0
            valor_frete = item.get("valor_frete", 0)

            if transportadora not in dados_agrupados:
                dados_agrupados[transportadora] = {
                    "total_cargas": 0,
                    "peso_total": 0.0,
                    "total_pagar": 0.0,
                }

            grupo = dados_agrupados[transportadora]
            grupo["total_cargas"] += 1
            grupo["peso_total"] += peso
            grupo["total_pagar"] += valor_frete

        dados_excel = []
        for transportadora, dados in sorted(dados_agrupados.items()):
            linha = {
    ...
    # Definir tipo_filtro para o template
    tipo_filtro = dados_corretos.get('tipo_filtro', 'semanal')
    
    return render_template(
        "/relatorios/relatorio_semanal/relatorio_sintetico_transportadora/relatorio_sintetico_transportadora.html",
        registros=registros,
        dados_corretos=dados_corretos,
        changelog=changelog,
        semanas_disponiveis=semanas_disponiveis,
