/**
 * JavaScript para Listagem de Receitas Avulsas
 * Funcionalidades: Seleção múltipla, cálculo de totais, controles de checkboxes, pesquisa e filtros
 */

class ReceitaAvulsaListagem {
    constructor() {
    ...
    inicializarElementos() {
        ...
        this.inputPesquisa = document.getElementById('search-receitas-avulsas');
    ...
    substituirTabelaEPaginacao(html) {
        ...
        const novoTbody = documento.querySelector('#corpo-tabela-receitas');
        const tBodyAtual = document.querySelector('#tabela-receitas-avulsas tbody');
        if (novoTbody && tBodyAtual) {
            tBodyAtual.innerHTML = novoTbody.innerHTML;
        }

        // Extrair nova paginação (card-footer)
        const novoCardFooter = documento.querySelector('#paginacao-receitas');
        const cardFooterAtual = document.querySelector('#paginacao-receitas');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.receitaListagem = new ReceitaAvulsaListagem();
    
    // Expor métodos úteis globalmente (opcional)
    window.obterItensSelecionados = () => window.receitaListagem.obterItensSelecionados();
    window.obterTotalSelecionado = () => window.receitaListagem.obterTotalSelecionado();
    window.limparSelecoes = () => window.receitaListagem.limparSelecoes();
});